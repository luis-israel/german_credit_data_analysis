...
_Account Balance_, por exemplo, parece positivamente correlacionado com a variável resposta. _Age_, por outro lado, não apresenta correlação linear significativa com a creditabilidade. Além disso, curiosamente, o valor da propriedade mais valiosa do cliente parece ser negativamente correlacionado à classe do pagador. Visualizar as proporções das classes relativas aos valores das preditoras pode gerar intuições mais específicas sobre as relações entre os dados:
...
```{r results = "markup"}
renderPlot({
  if (input$feature %in% c("Duration of Credit (month)", "Credit Amount", "Age (years)")) {
    ggplot(german_credit, aes(x = get(input$feature), fill = `Creditability`)) + geom_density(alpha = 0.3)
  } else {
    ggplot(german_credit, aes(x = get(input$feature), fill = `Creditability`)) + geom_histogram(alpha = 1, aes(y = ..density..))
  }
})
```

Os histogramas e gráficos de barra favorecem algumas interpretações dos dados. Pelo menos metade das preditoras parece ter algum efeito na resposta. Contudo, algumas das observações mais notáveis são:
...
Um último ponto importante antes do início da modelagem é a análise da proporção das classes na variável resposta:
...
Onde TP = bons pagadores classificados como bons pagadores, TN = maus pagadores classificados como maus pagadores, FP = maus pagadores classificados como bons pagadores, FN = bons pagadores classificados como maus pagadores. Accuracy = (TP+TN)/(TP+FP+FN+TN), Precision = TP/(TP+FP), Recall = TP/(TP+FN), F1 = (2 x Recall x Precision)/(Recall + Precision). Essas métricas, aliadas a conhecimento sobre domínio e gerenciamento de risco, devem auxiliar a tomada de decisão dos bancos na concessão dos empréstimos.